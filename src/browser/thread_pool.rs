...
pub fn run_bg(future: Fut) -> Result<(), JsValue> {
    let pool = WorkerPool::new(1)?;
    pool.run(|| wasm_bindgen_futures::spawn_local(future))?;
    Ok(())
}
...
fn spawn() -> Result<Worker, JsValue> {
